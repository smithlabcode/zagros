...
static void generate_position_weight_matrix(gsl_rng *gr, size_t n, size_t m,
    double target_ic, vector<vector<double> > &pwm) {
  double ic = 0;
  double sum = 0;
...
//  cerr << "Average IC : " << sum/pwm.size() << endl;
}
...
int main(int argc, const char **argv) {
  ...
  try {
    ...
    double target_ic = 1.0;
    size_t n_secondary_motifs = 5;
    ...
    opt_parse.add_opt(
        ...
        pwm_filename);
    opt_parse.add_opt(
        "n-sec-motifs", 'c', "number of secondary motifs in the data", false,
        n_secondary_motifs);
    opt_parse.add_opt(
    ...
    vector<vector<vector<double> > > pwm(n_secondary_motifs + 1);
    if (pwm_filename != "NA")
      read_position_weight_matrix(pwm_filename, pwm[n_secondary_motifs]);
    else
      generate_targeted_position_weight_matrix(
          gr, motif_width, smithlab::alphabet_size, target_ic,
          pwm[n_secondary_motifs]);
    vector<vector<double> > motif = pwm[n_secondary_motifs];
    string dout = pwm2dme(motif);
    for (size_t i = 0; i < pwm[n_secondary_motifs].size(); ++i)
      std::partial_sum(
          pwm[n_secondary_motifs][i].begin(), pwm[n_secondary_motifs][i].end(),
          pwm[n_secondary_motifs][i].begin());

    for (size_t i = 0; i < n_secondary_motifs; ++i) {
      generate_targeted_position_weight_matrix(
          gr, motif_width, smithlab::alphabet_size,
          target_ic/* - (0.1 * (i + 1))*/, pwm[i]);
      vector<vector<double> > motif = pwm[i];
      string dout = pwm2dme(motif);
      for (size_t j = 0; j < pwm[i].size(); ++j)
        std::partial_sum(pwm[i][j].begin(), pwm[i][j].end(), pwm[i][j].begin());
    }

    vector<vector<size_t> > indicators(n_secondary_motifs + 1);
    vector<vector<GenomicRegion> > diagnostic_events(n_secondary_motifs + 1);
    ...
    for (size_t i = 0; i <= n_secondary_motifs; ++i) {
      simulate_rnas(
          n_des_per_region, geo_delta_param, geo_rate_param, rng, regions,
          pwm[i], diagnostic_events[i], sequences, indicators[i]);
    }
    // Output the diagnostic events BED file and the updated sequences
    std::ofstream de_out(string(outfile + ".bed").c_str());
//    copy(diagnostic_events[n_secondary_motifs].begin(), diagnostic_events[n_secondary_motifs].end(),
//	 std::ostream_iterator<GenomicRegion>(de_out, "\n"));
    copy(
        diagnostic_events[n_secondary_motifs].begin(),
        diagnostic_events[n_secondary_motifs].begin()
            + diagnostic_events[n_secondary_motifs].size() / 2,
        std::ostream_iterator<GenomicRegion>(de_out, "\n"));
    for (size_t i = 0; i < n_secondary_motifs; ++i) {
      copy(
          diagnostic_events[i].begin() + diagnostic_events[i].size() / 2
              + (i * diagnostic_events[i].size()
                  / (2 * (n_secondary_motifs + 1))),
          diagnostic_events[i].begin() + diagnostic_events[i].size() / 2
              + ((i + 1) * diagnostic_events[i].size()
                  / (2 * (n_secondary_motifs + 1))),
          std::ostream_iterator<GenomicRegion>(de_out, "\n"));
    }
