...
string
Model::determineStartingPoint_best_kmer(const vector<string> &sequences) {
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    for (size_t k = 0; k < I[i].size(); k++) {
      ...
      if (D[i].size() > 0) {
        ...
        numerator[k] = (0.8 * numerator[k]
            + 0.2 * ((power * log(1 - p)) + (D[i].size() * log(p))));
        //--------
        numerator[k] *= (gamma / (S[i].length() - M.size() + 1));
        //--------
      }
...
void Model::expectation_maximization(const size_t max_iterations,
  ...
  string starting_point = determineStartingPoint_best_kmer(S);
