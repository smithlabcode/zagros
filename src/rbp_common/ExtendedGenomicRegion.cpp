...
void convertBowtieExtra(string &extra) {
...
void read_rmap_output(string filename, vector<ExtendedGenomicRegion> &regions) {
...
void read_bowtie_output(string filename,
...
void read_novoalign_output(string filename,
    vector<ExtendedGenomicRegion> &regions) {
  static const size_t buffer_size = 10000;
...
void read_piranha_output(string filename,
    vector<ExtendedGenomicRegion> &regions) {

  vector<GenomicRegion> the_regions;
  ReadBEDFile(filename, the_regions);
  for (size_t i = 0; i < the_regions.size(); i += 1) {
    if (the_regions[i].get_width() <= 500) {
      ExtendedGenomicRegion gr(
          the_regions[i].get_chrom(),
          the_regions[i].get_start() + int(the_regions[i].get_width() / 2)
              - 250,
          the_regions[i].get_end() - int(the_regions[i].get_width() / 2) + 250,
          the_regions[i].get_name(), the_regions[i].get_score(),
